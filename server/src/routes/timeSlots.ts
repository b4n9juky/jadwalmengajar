...
router.post('/', async (req, res) => {
  ...
  const data = { id: id || uuid(), academicYearId, dayOfWeek: Number(dayOfWeek), startTime, endTime, type: type || 'teaching' as const };
...
router.put('/', async (req, res) => {
  const { academicYearId, slots } = req.body as { academicYearId: string; slots: { id: string; dayOfWeek: number; startTime: string; endTime: string; type: string }[] };
  if (!academicYearId) {
    res.status(400).json({ error: 'academicYearId required' });
    return;
  }
  ...
  if (slots.length > 0) {
    const values = slots.map((s) => ({ id: s.id, academicYearId, dayOfWeek: Number(s.dayOfWeek), startTime: s.startTime, endTime: s.endTime, type: s.type as 'teaching' | 'break' }));
