...
router.put('/', async (req, res) => {
  const slots = req.body as { id: string; dayOfWeek: number; startTime: string; endTime: string; type: string }[];
  await db.delete(timeSlots);
  if (slots.length > 0) {
    const values = slots.map((s) => ({ id: s.id, dayOfWeek: Number(s.dayOfWeek), startTime: s.startTime, endTime: s.endTime, type: s.type as 'teaching' | 'break' }));
