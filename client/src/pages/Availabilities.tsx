...
import { useAvailabilities, useTeachers } from '../hooks/useQueries';
...
const empty: TeacherAvailability = { id: '', teacherId: '', dayOfWeek: 1, startTime: '07:30', endTime: '15:30', academicYearId: '' };

export default function Availabilities() {
  const { currentYear } = useAcademicYear();
  const ayId = currentYear?.id;
  const { data, save, remove } = useAvailabilities(ayId);
  ...
  const handleSave = async () => {
    await save.mutateAsync({ ...form, dayOfWeek: Number(form.dayOfWeek) });
    setShow(false);
  };

  const handleRemove = async (a: TeacherAvailability) => {
    if (window.confirm('Hapus ketersediaan ini?')) await remove.mutateAsync(a.id);
  };
